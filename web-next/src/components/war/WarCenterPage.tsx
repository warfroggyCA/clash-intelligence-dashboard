...
import { Button as BaseButton } from '@/components/new-ui/Button';
import { Input as BaseInput } from '@/components/new-ui/Input';
...
type WarButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement> & {
  variant?: 'primary' | 'secondary' | 'outline' | 'ghost' | 'destructive';
  size?: 'sm' | 'md' | 'lg';
  loading?: boolean;
};

const Button: React.FC<WarButtonProps> = ({ variant = 'primary', size = 'md', loading, disabled, className, style, children, ...rest }) => {
  const tone =
    variant === 'secondary'
      ? 'accentAlt'
      : variant === 'destructive'
  ...
  return (
    <BaseButton
      ...
      className={cx(
        'transition-transform enabled:hover:-translate-y-0.5 enabled:active:translate-y-0',
        loading && 'opacity-80',
        className,
      )}
      style={{ ...sizeStyle, ...outlineStyle, ...style }}
      {...rest}
    >
...
const GlassCard: React.FC<{ className?: string; children: React.ReactNode }> = ({ className, children }) => (
  <div
    className={cx('rounded-2xl border transition-all duration-150 hover:-translate-y-0.5', className)}
    style={{
      background: 'var(--card)',
...
const MetricCard: React.FC<MetricCardProps> = ({ label, value, tooltipKey }) => {
  const content = (
...
const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = ({ className, ...rest }) => (
  <BaseInput className={cx('bg-[var(--input)] text-slate-100', className)} {...rest} />
);

const WarSection: React.FC<React.ComponentProps<typeof CollapsibleSection>> = ({ className, headerClassName, contentClassName, ...rest }) => (
  <CollapsibleSection
    {...rest}
    className={cx('rounded-2xl border border-[var(--border-subtle)]', className)}
    headerClassName={cx('bg-[var(--panel)] text-slate-100 hover:bg-white/5', headerClassName)}
    contentClassName={cx('bg-[var(--card)]', contentClassName)}
  />
);
...
const WarCenterPage: React.FC = () => {
  ...
  const analysisStatus = (savedPlan?.analysisStatus ?? '').toLowerCase();
  const isAnalysisInFlight = runningAnalysis || ['queued', 'running'].includes(analysisStatus);
  ...
  if (permissionsError) {
    return (
      <>
        <div className="mx-auto flex w-full max-w-3xl flex-col gap-4 p-6">
          <GlassCard className="p-6 text-center text-sm text-rose-400">
    ...
    <>
      <div className="mx-auto flex w-full max-w-[1920px] flex-col gap-6 p-4 md:p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div
            className="flex flex-wrap items-center justify-end gap-3 rounded-2xl border p-4"
            style={{ borderColor: 'var(--border-subtle)', background: 'var(--panel)' }}
          >
            <div className="flex flex-col gap-1">
              <label className="flex items-center gap-2 rounded-full border border-[var(--border-subtle)] bg-white/5 px-3 py-1.5 text-xs font-semibold text-slate-200">
                <input
                  type="checkbox"
                  checked={useAI}
              ...
              {currentBriefingSource && (
                <span className="ml-3 text-[10px] uppercase tracking-wide text-slate-400">
                  Current: {currentBriefingSource}
                </span>
              )}
            </div>
            <div className="flex items-center gap-3 rounded-full border border-[var(--border-subtle)] bg-white/5 px-3 py-1.5 text-xs text-slate-200">
              <Tooltip content={TOOLTIP_CONTENT['Enrich Level']?.content ?? 'Controls how many top players to enrich'}>
                <span className="uppercase tracking-[0.2em] text-slate-400 inline-flex items-center gap-1">
                  Enrich
              ...
              <input
                ...
                className="h-1.5 w-28 accent-[var(--accent-alt)]"
              />
              <span className="w-12 text-right text-[11px] text-slate-400">Top {enrichLevel}</span>
            </div>
            <Button
              variant="secondary"
        ...
        <div className="space-y-6">
          <WarSection
            ...
            icon={<span className="text-xs text-slate-400">1</span>}
          >
            <div className="space-y-4">
              <GlassCard className="space-y-4 p-4">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs uppercase tracking-wide text-slate-400">Detected Opponent</label>
                    <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--panel)] px-3 py-2 text-sm">
                      {opponentProfile ? (
                        ...
                        <span className="text-slate-400">No opponent fetched yet.</span>
                      )}
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs uppercase tracking-wide text-slate-400">Town Hall Spread</label>
                    <div className="flex flex-wrap gap-2 text-xs text-slate-400">
                      {opponentThChips.length ? (
                        opponentThChips.map((chip) => (
                          <span key={chip.th} className="rounded-full border border-[var(--border-subtle)] bg-[var(--panel)] px-3 py-1">
                ...
                {(opponentProfileLoading || opponentProfileMessage || opponentProfileError) && (
                  <div className="flex flex-wrap items-center gap-3 text-xs">
                    {opponentProfileLoading && (
                      <span className="flex items-center gap-2 text-slate-400">
                        <Spinner className="text-[var(--accent-alt)]" />
                        Syncing opponent profile...
                      </span>
                    )}
                    {opponentProfileMessage && <span className="text-emerald-300">{opponentProfileMessage}</span>}
                    {opponentProfileError && <span className="text-rose-400">{opponentProfileError}</span>}
                  </div>
                )}
              </GlassCard>
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                <GlassCard className="p-4">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    {ourRosterError && <p className="text-sm text-rose-400">{ourRosterError}</p>}
                    {loadingOurRoster && (
                      <span className="flex items-center gap-2 text-xs text-slate-400">
                        <Spinner className="text-[var(--accent-alt)]" />
                        {ourRosterLoaded ? 'Refreshing roster...' : 'Loading roster...'}
                      </span>
                    )}
                    {ourRosterLoaded ? (
                      <RosterList
                      ...
                      />
                    ) : loadingOurRoster ? null : (
                      <p className="text-sm text-slate-400">Load your roster to start selecting participants.</p>
                    )}
                  </div>
                </GlassCard>
                ...
                <GlassCard className="p-4">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    {opponentError && <p className="text-sm text-rose-400">{opponentError}</p>}
                    {loadingOpponents && (
                      <span className="flex items-center gap-2 text-xs text-slate-400">
                        <Spinner className="text-[var(--accent-alt)]" />
                        {opponentRosterLoaded ? 'Refreshing opponent roster...' : 'Loading opponent roster...'}
                      </span>
                    )}
                    {opponentRosterLoaded ? (
                      <RosterList
                      ...
                      />
                    ) : loadingOpponents ? null : (
                      <p className="text-sm text-slate-400">Fetch the opponent roster to choose targets for enrichment.</p>
                    )}
                  </div>
                </GlassCard>
              </div>
          ...
          <WarSection
            ...
            icon={<span className="text-xs text-slate-400">2</span>}
          >
            <div className="grid gap-4 lg:grid-cols-2">
              <GlassCard className="p-4 space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <p className="text-xs uppercase tracking-wide text-slate-400">Our selections</p>
                    <p className="text-3xl font-semibold">{ourSelection.size}</p>
                    <p className="text-xs text-slate-400">Pick the attackers we&rsquo;ll assign.</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-wide text-slate-400">Opponent picks</p>
                    <p className="text-3xl font-semibold">{opponentSelection.size}</p>
                    <p className="text-xs text-slate-400">Target bases to enrich + analyze.</p>
                  </div>
                </div>
              ...
              <GlassCard className="p-4 space-y-3">
                ...
                {planLoading && <span className="text-xs text-slate-400">Loading saved plan…</span>}
                {planMessage && <span className="text-xs text-emerald-400">{planMessage}</span>}
                {planError && <span className="text-xs text-rose-400">{planError}</span>}
              </GlassCard>
            </div>
          </WarSection>
          ...
          <WarSection
            ...
            icon={<span className="text-xs text-slate-400">3</span>}
          >
            <GlassCard className="p-4 space-y-4">
              ...
              {analysisError && <p className="text-sm text-rose-400">{analysisError}</p>}
              {runningAnalysis && (
              ...
              {!matchup && (
                <p className="text-xs text-slate-400">Results will appear in Step 4 after the analysis completes.</p>
              )}
            </GlassCard>
          </WarSection>
          ...
          <WarSection
            ...
            icon={<span className="text-xs text-slate-400">4</span>}
          >
            <div className="space-y-4">
              {savedPlan && (
                <GlassCard className="p-4">
              ...
              {matchup ? (
                ...
                <GlassCard className="p-4 text-sm text-slate-400">
                  {isAnalysisInFlight ? (
                    <div className="flex items-center gap-2 text-slate-300">
                      <Spinner className="text-[var(--accent-alt)]" />
                      {analysisStatus === 'queued'
                        ? 'Analysis queued. Hang tight for results...'
                        : 'Analysis running...'}
                    </div>
                  ) : (
...
const RosterList: React.FC<{
  ...
  if (!displayRoster.length) {
    if (showSelectedOnly) {
      return <p className="text-sm text-slate-400">No participants selected yet.</p>;
    }
        ...
        return (
          <label
            ...
            className={cx(
              ...
              isSelected
                ? 'border-[var(--accent-alt)] bg-[var(--accent-alt)]/10'
                : 'border-[var(--border-subtle)] bg-[var(--panel)] hover:border-[var(--accent-alt)]',
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <input
                ...
                disabled={disabled}
                className="h-4 w-4 flex-shrink-0 accent-[var(--accent-alt)]"
              />
            </div>
...
const MatchupResult: React.FC<{
  ...
  const briefingTone = briefing
    ? briefing.confidenceBand === 'edge'
      ? 'text-emerald-300'
      : briefing.confidenceBand === 'underdog'
        ? 'text-rose-300'
        : 'text-slate-300'
    : 'text-slate-300';
  ...
  return (
    <div className="space-y-4">
      ...
      {briefing && (
        <GlassCard className="p-4 border border-[var(--accent-alt)]/30 bg-[var(--accent-alt)]/10 space-y-2">
          <div className={cx('text-xs uppercase tracking-[0.2em]', briefingTone)}>{briefingLabel}</div>
      ...
      {aiSuggestedOrder.length > 0 && (
        <GlassCard className="p-4 space-y-3 border border-[var(--accent-alt)]/30 bg-[var(--accent-alt)]/10">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold">AI Suggested Attack Order</h3>
            <span className="text-xs uppercase tracking-[0.3em] text-[var(--accent-alt)]">
              AI
            </span>
          </div>
      ...
      {suggestedOrder.length > 0 && (
        <GlassCard className="p-4 space-y-3 border border-[var(--accent)]/30 bg-[var(--accent)]/10">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold">Suggested Attack Order</h3>
            <span className="text-xs text-slate-400 uppercase tracking-[0.3em]">Heuristic</span>
          </div>
          <ol className="space-y-2 text-sm">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {slotBreakdown.map((slot) => (
                  <tr key={`slot-${slot.slot}`} className="border-b border-[var(--border-subtle)] last:border-b-0">
                    <td className="py-2">{slot.slot}</td>
                    <td className="py-2">
                      <div className="font-medium">{slot.ourName ?? slot.ourTag ?? '—'}</div>
                      <div className="text-xs text-slate-400">{slot.ourTag ?? '—'} • TH {slot.ourTH ?? '—'}</div>
                    </td>
                    <td className="py-2">
                      <div className="font-medium">{slot.opponentName ?? slot.opponentTag ?? '—'}</div>
                      <div className="text-xs text-slate-400">{slot.opponentTag ?? '—'} • TH {slot.opponentTH ?? '—'}</div>
                    </td>

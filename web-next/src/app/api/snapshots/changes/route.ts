...
export async function GET(req: NextRequest) {
  try {
    const logger = createRequestLogger(req, { route: '/api/snapshots/changes' });
    ...
    if (date) {
      ...
      const res = NextResponse.json<ApiResponse>({ success: true, data: changeSummary }, { headers: { 'Cache-Control': 'private, max-age=30' } });
      ...
      if (cached && (now - cached.timestamp) < CACHE_TTL) {
        const res = NextResponse.json<ApiResponse>({ success: true, data: cached.data }, { headers: { 'Cache-Control': 'private, max-age=30' } });
      ...
      const res = NextResponse.json<ApiResponse>({ success: true, data: allChanges }, { headers: { 'Cache-Control': 'private, max-age=30' } });
      logger.info('Served changes (fresh)', { clanTag, count: allChanges.length });
      return res;
    }
  } catch (error: any) {
    console.error('Error fetching changes:', error);
    return NextResponse.json<ApiResponse>({ success: false, error: error.message || 'Failed to fetch changes' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === 'save' && body.changeSummary) {
      ...
      return NextResponse.json<ApiResponse>({ success: true, message: "AI summary saved successfully" });
    }
    ...
    if (!clanTag || !date || !action) {
      return NextResponse.json<ApiResponse>({ success: false, error: "clanTag, date, and action are required" }, { status: 400 });
    }

    if (!['read', 'actioned'].includes(action)) {
      return NextResponse.json<ApiResponse>({ success: false, error: "Action must be 'read' or 'actioned'" }, { status: 400 });
    }
    ...
    if (!changeSummary) {
      return NextResponse.json<ApiResponse>({ success: false, error: "Change summary not found" }, { status: 404 });
    }
    ...
    return NextResponse.json<ApiResponse>({ success: true, data: changeSummary });
  } catch (error: any) {
    console.error('Error updating changes:', error);
    return NextResponse.json<ApiResponse>({ success: false, error: error.message || 'Failed to update changes' }, { status: 500 });
  }
}

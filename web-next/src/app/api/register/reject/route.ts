...
export async function POST(req: NextRequest) {
  const { json, logger } = createApiContext(req, 'register:reject');
  ...
  try {
    ...
    if (loadError) {
      logger.error('Failed to load registration for rejection', { error: loadError.message });
    ...
    if (updateError) {
      logger.error('Failed to reject registration', { error: updateError.message });
      return json({ success: false, error: 'Failed to reject registration' }, { status: 500 });
    }
    ...
    logger.error('Unexpected error rejecting registration', error);

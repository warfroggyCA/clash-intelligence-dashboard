...
export async function GET(request: NextRequest) {
  const { logger, json } = createApiContext(request, '/api/departures');
  try {
    ...
    const departures = await cached(['departures','list', clanTag], () => readDepartures(clanTag), 10);
    return json({ success: true, data: departures }, { headers: { 'Cache-Control': 'private, max-age=60' } });
  } catch (error: any) {
...
export async function POST(request: NextRequest) {
  const { json } = createApiContext(request, '/api/departures');
  try {

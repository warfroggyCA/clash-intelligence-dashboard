...
import RosterCardsView from '../page';

export default function NewRosterTablePage() {
  const { members, isLoading, error, isValidating, mutate } = useRosterData();
  const [search, setSearch] = useState('');
  const [isNarrow, setIsNarrow] = useState(false);

  useEffect(() => {
    const update = () => setIsNarrow(typeof window !== 'undefined' ? window.innerWidth < 1024 : false);
    update();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border" style={{ background: 'var(--card)', borderColor: 'var(--border-subtle)' }}>
        ...
        {isNarrow ? (
          <div className="px-4 py-4 text-sm text-slate-200 space-y-3">
            <p className="text-slate-200">Screen is narrow; showing the card layout for readability.</p>
            <RosterCardsView />
          </div>
        ) : (
                    ...
                    <thead className="text-xs uppercase tracking-[0.15em] text-slate-400">
                      <tr className="border-b border-white/5">
                        ...
                        {(['bk', 'aq', 'gw', 'rc', 'mp'] as const).map((heroKey) => (
                          <th key={`hero-head-${heroKey}`} className="px-2 py-3 text-center">
                            <img
                      ...
                      {filteredMembers.map((member) => {
                        ...
                        return (
                          <tr key={member.tag} className="border-b border-white/5 hover:bg-white/[0.03]">
                            <td className="px-2 py-3">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                  ...
                                  <span className="text-[12px] text-slate-400">{member.tag}</span>
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="px-2 py-3 font-semibold text-white text-right tabular-nums">{trophies.toLocaleString()}</td>
                            <td className="px-2 py-3 text-right tabular-nums font-semibold" style={{ color: '#34d399' }}>
                              {(member.donations ?? 0).toLocaleString()}
                            </td>
                            <td className="px-2 py-3 text-right tabular-nums">
                              {(member.donationsReceived ?? 0).toLocaleString()}
                            </td>
                            <td className="px-2 py-3 font-semibold text-right tabular-nums" style={{ color: rushTone(rushValue) }}>
                              {formatRush(rushValue)}
                            </td>
                            <td className="px-2 py-3 font-semibold text-white text-right tabular-nums">{Math.round(activity.score)}</td>
                            {(['bk', 'aq', 'gw', 'rc', 'mp'] as const).map((heroKey) => {
